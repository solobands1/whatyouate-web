...
function DownloadButton({ size = "lg" }: { size?: "lg" | "sm" }) {
  ...
  return (
    <a
      href={APP_STORE_URL}
      className="inline-flex items-center justify-center gap-2.5 rounded-full font-semibold tracking-[-0.01em] transition-opacity hover:opacity-80 active:opacity-60"
      style={{
        background: "#1F2937",
        color: "#fff",
        fontSize: lg ? 15 : 14,
        paddingLeft: lg ? 40 : 32,
        paddingRight: lg ? 40 : 32,
        paddingTop: lg ? 14 : 12,
        paddingBottom: lg ? 14 : 12,
        minWidth: lg ? 220 : 200,
      }}
    >
...
export default function Home() {
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[15px] font-semibold tracking-[-0.01em]" style={{ color: "#1F2937" }}>
            WhatYouAt<span className="relative inline-block">e
              <span className="absolute right-0 translate-x-[10px] text-[9px] font-semibold" style={{ top: 0, color: "#9CA3AF" }}>AI</span>
            </span>
          </span>
        </div>
      </header>
      ...
      {/* ── HERO ─────────────────────────────────────────────── */}
      <section
        className="flex flex-col md:flex-row items-center justify-center gap-12 md:gap-16 px-8 md:min-h-[82vh]"
      ...
      >
        <div className="flex flex-col items-center md:items-start text-center md:text-left max-w-[440px]">
          ...
          >
            <span className="h-1.5 w-1.5 rounded-full" style={{ background: "#6FA8FF" }} />
          ...
          <h1
            ...
            style={{ fontSize: "clamp(38px, 6vw, 56px)", marginBottom: 20, color: "#1F2937" }}
          >
            Eat Confidently<span style={{ color: "#6FA8FF" }}> | </span>Feel Better
          </h1>

          {/* Subline */}
          <p
            className="text-[18px] leading-[1.7]"
            style={{ marginBottom: 36, color: "#6B7280", maxWidth: 360 }}
          >
          ...
          <div className="flex flex-col items-center md:items-start" style={{ gap: 12 }}>
            ...
            <p className="text-[12px]" style={{ color: "#9CA3AF" }}>
              7-day Pro trial included · iPhone only
            </p>
          </div>
        </div>
      ...
      <section
        className="px-8"
        style={{ paddingTop: 64, paddingBottom: 64, borderTop: "1px solid rgba(31,41,55,0.08)" }}
      >
        <p
          className="mx-auto text-center text-[18px] leading-[1.85]"
          style={{ maxWidth: 580, color: "#6B7280" }}
        >
      ...
      <section
        ...
        style={{ paddingTop: 64, paddingBottom: 64, borderTop: "1px solid rgba(31,41,55,0.08)" }}
      >
        <div className="mx-auto flex flex-col" style={{ maxWidth: 560, gap: 22 }}>
          {features.map((f) => (
            <div key={f} className="flex items-start" style={{ gap: 16 }}>
              ...
              <p className="text-[16px] leading-[1.65]" style={{ color: "#1F2937" }}>{f}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      >
        <div className="mx-auto flex flex-col items-center" style={{ maxWidth: 440, gap: 16 }}>
          ...
          <p className="text-[16px]" style={{ color: "#6B7280", marginBottom: 8 }}>
      ...
      <footer
        className="px-8"
        style={{ paddingTop: 40, paddingBottom: 40, borderTop: "1px solid rgba(31,41,55,0.08)" }}
      >
        <div className="mx-auto max-w-4xl flex flex-col items-center gap-6 text-center">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-semibold tracking-[-0.01em]" style={{ color: "#9CA3AF" }}>
              WhatYouAt<span className="relative inline-block">e
                <span className="absolute right-0 translate-x-[10px] text-[9px] font-semibold" style={{ top: 0, color: "#9CA3AF" }}>AI</span>
              </span>
            </span>
          </div>
          <div className="flex gap-8 text-[13px]" style={{ color: "#9CA3AF" }}>
            <Link href="/privacy" className="hover:text-[#6B7280] transition-colors">Privacy Policy</Link>
            <a href="mailto:support@whatyouate.app" className="hover:text-[#6B7280] transition-colors">Support</a>
          </div>
          <span className="text-[12px]" style={{ color: "#9CA3AF" }}>© {new Date().getFullYear()} WhatYouAte. All rights reserved.</span>
        </div>
      </footer>
